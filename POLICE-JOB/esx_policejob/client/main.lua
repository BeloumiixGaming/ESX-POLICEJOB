...
RegisterNetEvent('esx_policejob:loadLicenses')
AddEventHandler('esx_policejob:loadLicenses', function(licenses)
	Licenses = licenses
end)
	...
	TriggerEvent('skinchanger:getSkin', function(skin)
		...
		if skin.sex == 0 then
			...
			if Config.Uniforms[job].male ~= nil then
				...
				ESX.ShowNotification("~g~~h~Début~w~ de service ~q~Policier~w~ !")
...
function OpenCloakroomMenu()
	...
	elseif grade == 'bac' then
		table.insert(elements, {label = _U('police_wear'), value = 'bac_wear'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		if data.current.value == 'citizen_wear' then
			...
			if Config.EnableNonFreemodePeds then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					...
					ESX.ShowNotification("~r~~h~Fin~w~ du service~q~ Policier~w~ !")
		...
		if
			...
			data.current.value == 'bac_wear' or
...
function OpenArmoryMenu(station)
  ...
  if Config.EnableArmoryManagement then
    ...
    local elements = {
	  ...
	  --{label = ('Achetez Lience Permis Port Armes'),     value = 'get_radio'},
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
		...
		if data.current.value == 'get_radio' then
			TriggerServerEvent('esx_policejob:buyItem1')
		end
...
function OpenPoliceActionsMenu()
	...
	'default', GetCurrentResourceName(), 'police_actions',
	{
		...
		elements = {
			...
			--{label = "Menu Prison",               value = 'jail_menu'} -- You add this line
		}
	}, function(data, menu)
				...
				elseif DoesEntityExist(vehicle) then
					...
					elseif action == 'hijack_vehicle' then
						if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.0) then
							...
							exports['progressBars']:startUI(20000, "Crochetage en cours")
							Citizen.Wait(20000)
...
function OpenGetStocksMenu1()
  ...
  ESX.TriggerServerCallback('esx_policejob:getStockItems1', function(items)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsHandcuffed then
			...
			DisableControlAction(2, Keys['X'], true) -- Levé les mains
			DisableControlAction(2, Keys['G'], true) -- Croiser les bras
			DisableControlAction(2, Keys['Y'], true) -- Croisé les bras vers le bas
			...
			DisableControlAction(2, Keys['F10'], true) -- Téléphone
			DisableControlAction(2, Keys['V'], true) -- Disable changing view
			DisableControlAction(2, Keys['P'], true) -- Disable pause screen
			DisableControlAction(2, 59, true) -- Disable steering in vehicle
			DisableControlAction(2, Keys['LEFTCTRL'], true) -- Disable going stealth
			DisableControlAction(2, Keys['LEFTALT'], true) -- Désactive animation RADIO
...
local blips = {
	...
	{title="~b~Commissariat", colour=63, id=60, x = 453.5, y = -984.98, z = 35.29}
	}
	Citizen.CreateThread(function()
...
function SendToCommunityService(player)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'Community Service Menu', {
		title = "Menu des services communautaires",
	}, function (data2, menu)
		local community_services_count = tonumber(data2.value)
		
		if community_services_count == nil then
			ESX.ShowNotification('Nombre de Service Invalide.')
		else
